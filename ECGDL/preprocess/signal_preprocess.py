...
def remove_baseline_wander(signals: List[float],
                           types: str,
                           sample_rate: int = LEAD_SAMPLING_RATE,
    ...
    if types == 'butterworth_highpass':
        ...
        filtered_signal, _, _ = ecg.st.filter_signal(signals, 'butter', 'highpass', 2, 1, sample_rate)
...
def remove_highfreq_noise(signals: List[float], types: str, sample_rate: int = LEAD_SAMPLING_RATE) -> List[float]:
    ...
    if types == 'butterworth_lowpass':
        filtered_signal, _, _ = ecg.st.filter_signal(signals, 'butter', 'lowpass', 12, 35, sample_rate)
    elif types == 'fir':
        filtered_signal, _, _ = ecg.st.filter_signal(signals, 'FIR', 'lowpass', 12, 35, sample_rate)
    else:
...
def preprocess_leads(lead: List[float]) -> List[float]:
    ...
    # TODO: add sampling ratio args
    corrected_signal = remove_baseline_wander(lead, 'butterworth_highpass')
