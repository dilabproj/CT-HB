...
def train_model_triplet(model: nn.Module,  # pylint: disable=too-many-locals
    ...
    for epoch in epochs:
        ...
        for anchor, positive, negative in training_data:
            if positive.dim() == 4:
                _, _, c, w = positive.size()
                positive = positive.view(-1, c, w)
            if negative.dim() == 4:
                _, _, c, w = negative.size()
                negative = negative.view(-1, c, w)
            ...
            if loss_function is not None:
                batch_loss = loss_function(anchor_output, positive_output, negative_output)
            else:
            ...
            training_data.set_description(f'Train loss: {train_loss / total_batches:.8f}')
