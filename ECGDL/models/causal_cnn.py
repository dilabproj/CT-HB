...
class CausalConvolutionBlock(torch.nn.Module):
    ...
    def forward(self, x):  # pylint: disable=arguments-differ
        ...
        # logger.debug("CUDA memory: %2f GB", torch.cuda.memory_stats()['active_bytes.all.current']/1024**3)
        res = x if self.upordownsample is None else self.upordownsample(x)
        if self.relu is None:
            return out_causal + res
        else:
            return self.relu(out_causal + res)  # pylint: disable=not-callable
...
class CausalCNNEncoder(torch.nn.Module):
    ...
    @param kernel_size Kernel size of the applied non-residual convolutions.
    """
    def __init__(self, in_channels, channels, depth, reduced_size,  # pylint: disable=unused-argument
                 out_channels, kernel_size, **kwargs):
